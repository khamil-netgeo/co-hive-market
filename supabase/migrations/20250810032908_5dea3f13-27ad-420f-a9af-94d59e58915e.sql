...
create policy if not exists "Public read short videos"
  on storage.objects
  for select
  using (bucket_id = 'short-videos');

create policy if not exists "Users can upload own short videos"
  on storage.objects
  for insert
  with check (
...
create policy if not exists "Users can update own short videos"
  on storage.objects
  for update
  using (
    bucket_id = 'short-videos'
...
create policy if not exists "Users can delete own short videos"
  on storage.objects
  for delete
  using (
    bucket_id = 'short-videos'
...
create policy if not exists "Public view published shorts"
  on public.short_posts
  for select
  using (status = 'published');

create policy if not exists "Owners view own shorts"
  on public.short_posts
  for select
  using (user_id = auth.uid());

create policy if not exists "Users create own shorts"
  on public.short_posts
  for insert
  with check (user_id = auth.uid());

create policy if not exists "Owners/admins update shorts"
  on public.short_posts
  for update
...
create policy if not exists "Owners/admins delete shorts"
  on public.short_posts
  for delete
  using (user_id = auth.uid() or has_role(auth.uid(), 'admin') or has_role(auth.uid(), 'superadmin'));
