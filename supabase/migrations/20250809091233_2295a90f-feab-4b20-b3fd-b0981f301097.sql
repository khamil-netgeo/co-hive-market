...
CREATE POLICY "Users can delete product images in own folder"
ON storage.objects
FOR DELETE
USING (
  bucket_id = 'product-images' AND auth.uid()::text = (storage.foldername(name))[1]
...
CREATE POLICY "Users can upload service images to own folder"
ON storage.objects
FOR INSERT
WITH CHECK (
...
CREATE POLICY "Users can update service images in own folder"
ON storage.objects
FOR UPDATE
USING (
  bucket_id = 'service-images' AND auth.uid()::text = (storage.foldername(name))[1]
...
CREATE POLICY "Users can delete service images in own folder"
ON storage.objects
FOR DELETE
USING (
  bucket_id = 'service-images' AND auth.uid()::text = (storage.foldername(name))[1]
