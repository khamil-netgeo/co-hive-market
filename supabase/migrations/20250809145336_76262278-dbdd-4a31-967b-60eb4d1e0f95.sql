...
-- Only superadmins (and admins optionally) manage categories
CREATE POLICY IF NOT EXISTS "Superadmins manage categories"
ON public.categories
FOR ALL
...
CREATE POLICY IF NOT EXISTS "Public select product categories for active products"
...
CREATE POLICY IF NOT EXISTS "Vendors manage product categories"
ON public.product_categories
FOR ALL
USING (EXISTS (
  SELECT 1 FROM public.products p
  ...
  WHERE p.id = product_categories.product_id
    AND (v.user_id = auth.uid() OR has_role(auth.uid(), 'admin') OR has_role(auth.uid(), 'superadmin'))
))
WITH CHECK (EXISTS (
  SELECT 1 FROM public.products p
  JOIN public.vendors v ON v.id = p.vendor_id
...
CREATE POLICY IF NOT EXISTS "Public select service categories for active services"
...
CREATE POLICY IF NOT EXISTS "Vendors manage service categories"
ON public.service_categories
FOR ALL
USING (EXISTS (
  SELECT 1 FROM public.vendor_services s
