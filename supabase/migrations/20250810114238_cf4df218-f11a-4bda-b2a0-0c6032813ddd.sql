-- Fix previous migration by recreating policies and triggers without IF NOT EXISTS

-- Chat threads table (already created earlier). Ensure RLS and policies.
ALTER TABLE public.chat_threads ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Participants and admins can view chat threads" ON public.chat_threads;
CREATE POLICY "Participants and admins can view chat threads"
...
DROP POLICY IF EXISTS "Participants and admins can insert chat threads" ON public.chat_threads;
CREATE POLICY "Participants and admins can insert chat threads"
ON public.chat_threads
FOR INSERT
WITH CHECK (
  (buyer_user_id = auth.uid())
...
DROP POLICY IF EXISTS "Participants and admins can update chat threads" ON public.chat_threads;
CREATE POLICY "Participants and admins can update chat threads"
ON public.chat_threads
FOR UPDATE
USING (
  (buyer_user_id = auth.uid())
...
DROP POLICY IF EXISTS "Admins can delete chat threads" ON public.chat_threads;
...
-- Chat messages table (ensure exists)
...
CREATE POLICY "Owner vendor or admins can view tickets"
...
DROP POLICY IF EXISTS "Owner updates own ticket or admins" ON public.support_tickets;
CREATE POLICY "Owner updates own ticket or admins"
ON public.support_tickets
FOR UPDATE
USING (
  created_by = auth.uid()
...
DROP POLICY IF EXISTS "View messages for your tickets" ON public.support_messages;
CREATE POLICY "View messages for your tickets"
...
DROP POLICY IF EXISTS "Insert messages on your tickets" ON public.support_messages;
CREATE POLICY "Insert messages on your tickets"
ON public.support_messages
FOR INSERT
WITH CHECK (
  sender_user_id = auth.uid() AND
