-- Structured fields for vendor_services and service add-ons (retry without IF NOT EXISTS on policies)
...
DO $$ BEGIN
  DROP POLICY IF EXISTS "Public can view addons for active services" ON public.service_addons;
  CREATE POLICY "Public can view addons for active services"
...
DO $$ BEGIN
  DROP POLICY IF EXISTS "Vendors/admins view own addons" ON public.service_addons;
  CREATE POLICY "Vendors/admins view own addons"
  ON public.service_addons
  FOR SELECT
  USING (
    EXISTS (
...
DO $$ BEGIN
  DROP POLICY IF EXISTS "Vendors/admins insert addons" ON public.service_addons;
  CREATE POLICY "Vendors/admins insert addons"
  ON public.service_addons
  FOR INSERT
  WITH CHECK (
    EXISTS (
...
DO $$ BEGIN
  DROP POLICY IF EXISTS "Vendors/admins update addons" ON public.service_addons;
  CREATE POLICY "Vendors/admins update addons"
  ON public.service_addons
  FOR UPDATE
  USING (
    EXISTS (
...
DO $$ BEGIN
  DROP POLICY IF EXISTS "Vendors/admins delete addons" ON public.service_addons;
  CREATE POLICY "Vendors/admins delete addons"
  ON public.service_addons
  FOR DELETE
  USING (
    EXISTS (
