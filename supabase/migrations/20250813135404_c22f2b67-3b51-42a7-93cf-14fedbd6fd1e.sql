...
CREATE POLICY IF NOT EXISTS "View related shipments"
...
CREATE POLICY IF NOT EXISTS "System can insert shipments"
ON public.order_shipments
FOR INSERT
WITH CHECK (true);

-- Vendors or admins can update shipments
CREATE POLICY IF NOT EXISTS "Vendors/admins update shipments"
ON public.order_shipments
FOR UPDATE
USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "Admins delete shipments"
ON public.order_shipments
FOR DELETE
USING (
