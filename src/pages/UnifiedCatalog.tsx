            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                {filtered.map((item) => {
                  ...
                  return (
                    <Card key={`${item.type}-${item.id}`} className="hover:shadow-elegant transition-shadow">
                      ...
                      <CardContent className="grid gap-3">
                        ...
                        <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                          ...
                          {item.type === 'product' && item.pickup_lat && item.pickup_lng && loc && (
                            <span>Distance: {haversineKm(loc.lat, loc.lng, item.pickup_lat, item.pickup_lng).toFixed(1)} km</span>
                          )}
                        </div>
                        ...
                        <div className="text-2xl font-semibold">
                          {discounted != null ? (
                            <div className="flex items-baseline gap-2">
                              <span>{fmtPrice(discounted, item.currency)}</span>
                        ...
                        {item.type === 'service' && (
                          <div className="grid gap-2">
                            <label className="text-xs text-muted-foreground" htmlFor={`dt-${item.id}`}>
                        ...
                        <div className="flex flex-col sm:flex-row gap-2">
                          {item.type === 'product' ? (
                            <>
                              <Button variant="secondary" onClick={() => addToCart(item)} className="w-full sm:w-auto">
                                Add to cart
                              </Button>
                              <Button variant="hero" onClick={() => buyNow(item)} className="w-full sm:w-auto">
                                Buy now
                              </Button>
                            </>
                          ) : (
                            <Button variant="hero" onClick={() => bookService(item)} className="w-full">
                              Book & Pay
                            </Button>
                          )}
                        </div>
