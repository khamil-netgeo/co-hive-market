...
const Categories = ({ onGetStarted }: CategoriesProps) => {
  const { data: categories, isLoading } = useLandingCategories();
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="animate-pulse">
              <div className="mx-auto mb-4 h-12 w-12 rounded-lg bg-muted"></div>
    ...
    <section id="categories" className="container py-16 md:py-20">
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-3 grid-fade-in">
        {categories?.map((category) => {
          const IconComponent = getLucideIcon(category.icon_name);
          return (
            <div key={category.id} className="feature-card animate-fade-in-up group text-center">
